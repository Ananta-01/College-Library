...
function showBooks() {
    ...
    bookObj.forEach(function (ele, index) {
        rowData += `
        <tr class="colours1">
        <td class="colours">${index +1}</td>
        <td class="coloursName">${ele.name}</td>
        <td class="colours txt">${ele.author}</td>
        <td class="colours txt">${ele.type}</td>
        <td class="colours"><button id="${index}" onclick="deleteBook(this.id)" class="btn btn-primary delete">Delete</button></td>
        </tr>
        `
    })
...
search.addEventListener("input", function(){
    ...
    let colours1 = document.getElementsByClassName('colours1');
    Array.from(colours1).forEach(function(element){
        let card = element.getElementsByClassName('coloursName')[0].innerText;
